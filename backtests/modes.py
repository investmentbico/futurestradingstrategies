...
# Conservative params for prop firm
fast_ema = 21
slow_ema = 50
rsi_len = 14
rsi_long_thresh = 60  # back to original
rsi_short_thresh = 40  # back to original
atr_mult_sl = 2.0  # back to original
risk_percent = 0.6  # increased for higher returns
...
for mode_name, mode_type in modes.items():
    # Filters
    df['momentum'] = df['close'] > df['close'].shift(5)  # back to original
    df['volatility'] = df['atr'] > df['atr'].rolling(20).mean()
    df['trend'] = df['adx'] > 30  # stronger
    df['advanced'] = (df['bar_range_pct'] > 0.5) & ((df['rsi'] < 30) | (df['rsi'] > 70))
    ...
    if mode_type == 'momentum':
        long_signal = long_base & df['momentum']
        short_signal = short_base & ~df['momentum']
    elif mode_type == 'volatility':
        long_signal = long_base & df['volatility']
        short_signal = short_base & df['volatility']
    elif mode_type == 'trend':
        long_signal = long_base & df['trend']
        short_signal = short_base & df['trend']
    elif mode_type == 'all':
        long_signal = long_base & df['momentum'] & df['volatility'] & df['trend']
        short_signal = short_base & ~df['momentum'] & df['volatility'] & df['trend']
    elif mode_type == 'advanced':
        long_signal = long_base & df['advanced']
        short_signal = short_base & df['advanced']
    ...
    winning_trades = []
    losing_trades = []
    all_trades = []  # list of dicts: {'entry_time':, 'exit_time':, 'pnl':, 'type': 'long'/'short', 'win': True/False}
    daily_drawdowns = []
    daily_min = equity
...
# Print top 3
sorted_results = sorted(results.items(), key=lambda x: x[1]['Total PNL'], reverse=True)
for i, (mode, metrics) in enumerate(sorted_results[:3]):
    print(f"Top {i+1}: {mode} Mode")
    print(f"  Total PNL: ${metrics['Total PNL']:.2f}")
    print(f"  Win Rate: {metrics['Win Rate %']:.2f}%")
    print(f"  Profit Factor: {metrics['Profit Factor']:.2f}")
    print(f"  Max Daily Drawdown: ${metrics['Max Daily Drawdown']:.2f}")
    print()