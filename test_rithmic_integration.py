#!/usr/bin/env python3
"""
Quick test of Rithmic API integration
"""
...
def test_rithmic_api():
    print("🧪 Testing Rithmic API integration...")

    # Test demo mode (should use mock data)
    print("📊 Testing demo mode...")
    api = TestRithmicAPI(is_demo=True)

    # Get current price
    price = api.get_current_price('MNQ')
    print(f"💰 Current MNQ price: ${price:.2f}")

    # Get market data
    data = api.get_market_data('MNQ')
    ...
    # Test live mode (will try to connect but fail gracefully)
    print("🔴 Testing live mode (should fall back to mock)...")
    api_live = TestRithmicAPI(is_demo=False)

    price_live = api_live.get_current_price('MNQ')
    print(f"💰 Live mode MNQ price: ${price_live:.2f}")

    print("✅ Rithmic API integration test completed successfully!")
