#!/usr/bin/env python3
"""
ES-Style Backtest - Apply ES 2min Success to MNQ, MES, NQ
========================================================

Uses the exact parameters that made ES 2min successful ($31K P&L, 203 trades)
Applied to MNQ, MES, NQ with same logic and risk management.

Strategy: Stoch-D Mean Reversion with EMA Trend Filter
Parameters: Exact same as successful ES 2min
Target: Similar performance across all symbols
"""
...
# =============================================================================
# CONFIGURATION - EXACT ES 2MIN PARAMETERS
# =============================================================================
...
# EXACT PARAMETERS FROM SUCCESSFUL ES 2MIN ($31K P&L, 203 trades, 53.2% win rate)
STRATEGY_PARAMS = {
    '2min': {
...
# =============================================================================
# STRATEGY LOGIC - EXACT SAME AS ES 2MIN SUCCESS
# =============================================================================
def generate_signals(df: pd.DataFrame, params: Dict) -> pd.DataFrame:
    df = df.copy()

    # Calculate indicators - exact same as ES 2min
    ...
    # Stoch-D signals - exact same levels as ES 2min
    ...
    # Entry signals - exact same logic
    df['long_signal'] = (
        df['trend_up'] &
        df['stoch_oversold'] &
        df['momentum_up']
...
# =============================================================================
# BACKTEST ENGINE - EXACT SAME AS COMPREHENSIVE
# =============================================================================
def run_single_backtest(symbol: str) -> Dict:
    timeframe = '2min'  # Only 2min since that's what worked for ES
    print(f"🔄 Backtesting {symbol} on {timeframe} timeframe (ES 2min Parameters)...")
...
# =============================================================================
def main():
    print("🚀 ES-STYLE BACKTEST - APPLY SUCCESS TO MNQ, MES, NQ")
    print("=" * 55)
    print("Using EXACT parameters from ES 2min success:")
    print("• $31,403 P&L, 203 trades, 53.2% win rate")
    print("• Stoch-D: (11, 3, 2) with levels 27/73")
    print("• EMA: 21/55 crossover")
    print("• ATR stops with 1.2x multiplier")
    ...
    print(f"🔄 Testing {len(symbols)} symbols with ES 2min parameters...")
    print()
    ...
    print("\n" + "=" * 80)
    print("🎯 ES-STYLE BACKTEST RESULTS")
    print("=" * 80)
    ...
    # Compare to ES baseline
    es_comparison = "VS ES 2min: $31,403 P&L, 203 trades, 53.2% win rate"
    print(f"   {es_comparison}")

    # Save results
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    results_file = f'results/es_style_backtest_{timestamp}.csv'
    ...
    if best_result['equity_curve']:
        ...
        plt.title(f'Equity Curve - {best_result["symbol"]} {best_result["timeframe"]} (ES-Style Parameters)')
        plt.xlabel('Trades')
        plt.ylabel('Equity ($)')
        ...
        chart_file = f'results/es_style_equity_curve_{timestamp}.png'
        plt.savefig(chart_file, dpi=150, bbox_inches='tight')
        print(f"📊 Chart saved to {chart_file}")
        plt.close()

    print("\n✅ ES-style backtest complete!")
