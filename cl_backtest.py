#!/usr/bin/env python3
"""
CL Strategy Backtest with Daily Historical Data
===============================================

Backtests the trading strategy using daily CL (WTI Oil) data.
Adapted for daily timeframe with appropriate parameters for oil futures.
"""
...
# =============================================================================
# STRATEGY PARAMETERS (Adapted for Daily CL)
# =============================================================================
...
RISK_PARAMS = {
    'CONTRACTS': 1,
    'MAX_LOSS_TRADE': 400.0,  # Higher for oil volatility
    'HARD_STOP_DOLLARS': 400.0,  # $400 hard stop for oil
    'ATR_MULTIPLIER': 2.0,
    'ATR_PERIOD': 14,
...
class CLBacktest:
    ...
    def download_data(self):
        ...
        print(f"📊 Loading local CL daily data...")

        import os
        data_file = "data/cl_daily.csv"
...
def main():
    ...
    print("🚀 CL BACKTEST: 1 CONTRACT - DAILY DATA")
    print("=" * 70)

    # Load data (last 90 days = 3 months)
    backtest = CLBacktest(symbol='CL', days=90)
    ...
    configs = [
        {
            'name': 'CL_OPTIMIZED',
            'contracts': 1,
            'hard_stop': 400,  # $400 hard stop for oil volatility
            'atr_mult': 2.0,
            'atr_period': 14,
            'tp_mult': 2.0,
            'description': 'Optimized params for CL daily data'
        }
    ]
    ...
    for config in configs:
        ...
        test_backtest = CLBacktest(symbol='CL', days=30)
